...
class Python3Recipe(TargetPythonRecipe):
    ...
    def get_recipe_env(self, arch=None, with_flags_in_cc=True):
        ...
        # HydroSmart fix: newer Clang (bundled with recent NDKs) turns an
        # implicit function declaration into a hard compile error in C99
        # mode. CPython's own ./configure auto-detects this compiler
        # capability and, if present, appends "-Werror=implicit-function-
        # declaration" to its Makefile's CFLAGS_NODIST *after* anything we
        # pass via CFLAGS above, so overriding CFLAGS alone can never win.
        # Android's libc headers don't declare setgrent/getgrent/endgrent
        # (used by CPython's Modules/grpmodule.c) for our target API level,
        # which trips this exact check and aborts the whole Android python3
        # build. Pre-seeding the autoconf cache variable below makes
        # ./configure skip its own detection test and never add that flag,
        # which is the supported way to override an AC_CACHE_CHECK result.
        # See configure.ac's "ac_cv_enable_implicit_function_declaration_error"
        # check for reference. Not (yet) fixed upstream in python-for-android.
        env['ac_cv_enable_implicit_function_declaration_error'] = 'no'
